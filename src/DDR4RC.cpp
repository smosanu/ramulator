...
string DDR4RC::standard_name = "DDR4RC";
string DDR4RC::level_str [int(Level::MAX)] = {"Ch", "Ra", "Bg", "Ba", "Ro", "Co"};

map<string, enum DDR4RC::Org> DDR4RC::org_map = {
    {"DDR4RC_2Gb_x4", DDR4RC::Org::DDR4RC_2Gb_x4}, {"DDR4RC_2Gb_x8", DDR4RC::Org::DDR4RC_2Gb_x8}, {"DDR4RC_2Gb_x16", DDR4RC::Org::DDR4RC_2Gb_x16},
    {"DDR4RC_4Gb_x4", DDR4RC::Org::DDR4RC_4Gb_x4}, {"DDR4RC_4Gb_x8", DDR4RC::Org::DDR4RC_4Gb_x8}, {"DDR4RC_4Gb_x16", DDR4RC::Org::DDR4RC_4Gb_x16},
    {"DDR4RC_8Gb_x4", DDR4RC::Org::DDR4RC_8Gb_x4}, {"DDR4RC_8Gb_x8", DDR4RC::Org::DDR4RC_8Gb_x8}, {"DDR4RC_8Gb_x16", DDR4RC::Org::DDR4RC_8Gb_x16},
};

map<string, enum DDR4RC::Speed> DDR4RC::speed_map = {
    {"DDR4RC_1600K", DDR4RC::Speed::DDR4RC_1600K}, {"DDR4RC_1600L", DDR4RC::Speed::DDR4RC_1600L},
    {"DDR4RC_1866M", DDR4RC::Speed::DDR4RC_1866M}, {"DDR4RC_1866N", DDR4RC::Speed::DDR4RC_1866N},
    {"DDR4RC_2133P", DDR4RC::Speed::DDR4RC_2133P}, {"DDR4RC_2133R", DDR4RC::Speed::DDR4RC_2133R},
    {"DDR4RC_2400R", DDR4RC::Speed::DDR4RC_2400R}, {"DDR4RC_2400U", DDR4RC::Speed::DDR4RC_2400U},
    {"DDR4RC_3200", DDR4RC::Speed::DDR4RC_3200},
};


DDR4RC::DDR4RC(Org org, Speed speed)
...
DDR4RC::DDR4RC(const string& org_str, const string& speed_str) :
    DDR4RC(org_map[org_str], speed_map[speed_str]) 
{
}

void DDR4RC::set_channel_number(int channel) {
  org_entry.count[int(Level::Channel)] = channel;
}

void DDR4RC::set_rank_number(int rank) {
  org_entry.count[int(Level::Rank)] = rank;
}

void DDR4RC::init_speed()
{
...
void DDR4RC::init_prereq()
{
    // RD
    prereq[int(Level::Rank)][int(Command::RD)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // REF
    prereq[int(Level::Rank)][int(Command::REF)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
    ...
    // PD
    prereq[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // SR
    prereq[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void DDR4RC::init_rowhit()
{
    // RD
    rowhit[int(Level::Bank)][int(Command::RD)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void DDR4RC::init_rowopen()
{
    // RD
    rowopen[int(Level::Bank)][int(Command::RD)] = [] (DRAM<DDR4RC>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
...
void DDR4RC::init_lambda()
{
    lambda[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::PRE)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PREA)] = [] (DRAM<DDR4RC>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                bank->state = State::Closed;
                bank->row_state.clear();
            }};
    lambda[int(Level::Rank)][int(Command::REF)] = [] (DRAM<DDR4RC>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RD)] = [] (DRAM<DDR4RC>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::WR)] = [] (DRAM<DDR4RC>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Bank)][int(Command::WRA)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PDE)] = [] (DRAM<DDR4RC>* node, int id) {
        for (auto bg : node->children)
            for (auto bank : bg->children) {
                if (bank->state == State::Closed)
                    continue;
    ...
    lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::PowerUp;};
    lambda[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::SelfRefresh;};
    lambda[int(Level::Rank)][int(Command::SRX)] = [] (DRAM<DDR4RC>* node, int id) {
        node->state = State::PowerUp;};
}


void DDR4RC::init_timing()
{
